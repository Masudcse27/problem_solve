...
void solve(){
    int n;cin>>n;
    int seg=(sqrt(n))+1;
    vector<int>v(n),sqd[seg];
    for(int i=0;i<n;i++){
        cin>>v[i];

        sqd[i/seg].push_back(v[i]);
    }
    for(int i=0;i<seg;i++){
        sort(sqd[i].begin(),sqd[i].end());
    }
    int q;cin>>q;
    //cout<<q<<endl;
    while (q--){
        ...
        if(s==e){
            ...
            for(int i=l;i<(s+1)*seg;i++){
                ans+=(v[i]>k);
            }
            for(int i=(e*seg)+1;i<=r;i++){
                ans+=(v[i]>k);
            }
            for(int i=s+1;i<=e;i++){
                int l=upper_bound(sqd[i].begin(),sqd[i].end(),k)-sqd[i].begin();
                ans+=sqd[i].size()-l-1;
            }
        }
        cout<<ans<<endl;
